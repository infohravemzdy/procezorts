...
describe('Procezor Example - Salary-Bonus-Barter', () => {
    const getTargetsFromDb = function(period: IPeriod): Array<ITermTarget> {
        ...
        const targets  = [
            ...
            ExampleTermTarget.zero(montCode, contract, position, variant1,
                ArticleCode.get(ExampleArticleConst.ARTICLE_PAYMENT_BONUS),
                ConceptCode.get(ExampleConceptConst.CONCEPT_AMOUNT_FIXED)),
            ExampleTermTarget.zero(montCode, contract, position, variant1,
                ArticleCode.get(ExampleArticleConst.ARTICLE_PAYMENT_BARTER),
                ConceptCode.get(ExampleConceptConst.CONCEPT_AMOUNT_FIXED)),
        ];
        return targets;
    };
    ...
    it(`concept should be valid, got ${factoryConcept}`, () => {
        ...
        expect(factoryConcept.code.value).to.equal(ExampleArticleConst.ARTICLE_TIMESHT_WORKING);
    });
    ...
    const testArticles: number[] = [80001, 80003, 80004, 80002, 80006, 80007, 80010, 80012, 80008, 80009, 80011, 80013];
